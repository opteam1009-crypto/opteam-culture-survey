 ...
 *
 * 두 계정 모두 제출된 응답을 전부 열람합니다. 예전에는 「인사책임자만 열람」 을
 * 고른 응답이 대표이사에게 가려졌지만, 조직 상태를 판단하려면 대표이사도 전부
 * 봐야 한다는 결정에 따라 가림을 없앴습니다.
 *
 * 그래서 열람 범위 선택은 이제 접근 제한이 아니라 "누구에게 먼저 닿기를 바라는지"
 * 를 알리는 표시입니다. 설문 화면 문구도 그렇게 바꿔 두었습니다. 응답자에게
 * 가려진다고 약속해 놓고 보는 일이 없도록, 이 배열을 줄일 때는 설문 화면 문구도
 * 함께 되돌려야 합니다.
 */
export const VISIBLE_TO: Record<Role, string[]> = {
  ceo: ["both", "ceo_only", "hr_only"],
